...
def find_highest_joltage_naive(battery_string:str):
    highest = -1
    next_highest = -1

    for i in range(len(battery_string)-1):
        b_i = int(battery_string[i])
        if(b_i > highest):
            next_highest = int(battery_string[i+1])
            highest = b_i
        elif(b_i > next_highest):
            next_highest = b_i

    if(int(battery_string[-1]) > next_highest):
        next_highest = int(battery_string[-1])

    return highest*10 + next_highest

def find_highest_joltage_sum(full_battery_list):
    joltages = []
    for battery_line in full_battery_list:
        joltages.append(find_highest_joltage_naive(battery_line))
...
"""
    ...
    joltages, s = find_highest_joltage_sum(test_data_list)
...
def main_1():
    ...
    joltages, s = find_highest_joltage_sum(data)
